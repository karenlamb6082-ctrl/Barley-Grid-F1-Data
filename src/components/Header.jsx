...
export default function Header({ currentView, setCurrentView }) {
  ...
  return (
    <header className="bg-f1-bg/90 backdrop-blur-xl border-b border-black/10 sticky top-0 z-50">
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-out ${menuOpen ? 'max-h-48 border-t border-black/[0.04]' : 'max-h-0'}`}>
        <nav className="px-4 py-3 space-y-1 bg-[#171717] text-white backdrop-blur-xl">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => handleNav(tab.id)}
