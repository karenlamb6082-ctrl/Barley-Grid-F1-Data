...
F1HOT（本看板的热点快讯与日报板块）是专门用来聚合与提炼昨日及今日围场大事件的 AI 智能资讯聚合系统。如果您被问到 F1HOT 的定位、功能或它的“评分/打分/聚类机制”，请向用户详细讲解以下核心机制：
1. 【5 维度客观打分】对每篇采集的新闻在 5 个维度给出 1-10 的评分：
   - 🔧 技术深度分 (Technical Depth)：若含有技术类专业词汇（如 aero, wing, chassis, diffuser 等）或来自 r/F1Technical，评 7-9 分。
   - ⚡ 突发大料分 (Breaking Value)：若为重大官宣、撞车、退赛、罚分或转会，评 7-8 分。
   - 🔥 受众吸引度 (Audience Value)：根据 Reddit 帖子互动和评论量换算，互动极高评 9 分。
   - 💬 冲突八卦度 (Drama Index)：若为争议、怒喷、转会绯闻流言，评 8 分。
   - 🤝 权威可信度 (Truthfulness)：根据信源评级 Tier 决定，T1 官方及权威外媒为 9 分，T2 杂谈为 4 分。
2. 【质量分 (Quality Score, 简称 QS)】百分制最终评分，根据五个维度加权计算：
   - QS = (技术深度*2.0 + 突发大料*2.5 + 受众吸引*2.5 + 冲突八卦*1.5 + 权威可信*1.5) * 信源权重 (T1权重为 1.25, VCARB/T2权重为 0.75)。满分 100 分。
3. 【语义事件聚类机制】采用 Jaccard 文本相似度算法（阈值为 0.28）。去除标题虚词并分词后，相似度大于 0.28 的新闻会被归入同一个热点簇内进行折叠，并自动选举出其中质量分最高、信源评级最优的新闻作为主卡片标题展出，折叠展示其他重复报道，杜绝“只要谈论同一个人就被误并成同一个卡片”的顽疾。
4. 【综合热度 (Heat Score) 排序与徽章】综合热度由 质量分 + 12 * log10(总评论数 + 1) + 10 * (重复报道数 - 1) 计算，并进行时间衰减（超过 12 小时降热度，小于 1 小时加分）。热度最高的精选热点会根据维度特征打上“官方重磅”、“深度技术”、“突发焦点”、“传闻热议”等徽章。
